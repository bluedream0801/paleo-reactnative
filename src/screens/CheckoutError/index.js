...
const CheckoutError = (props) => {
  ...
  const { sale_id, transaction_id } = route.params;
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      console.log('CURRENT SALE ID', route.params, sale_id, transaction_id);
  ...
  useFocusEffect(
    useCallback(() => {
      AppsFyler.logEvent('check_out_error', {
      
      });
      analytics().logEvent('checkout_error');
    }, [])
  );
    ...
    return (
      <ScrollView>
        <View style={[styles.successContainer, { paddingTop: insets.top}]}>
          ...
          <ChatUs style={{marginTop: 10, marginBottom: 20, width: '100%'}}></ChatUs>
          <Button
            onPress={() => {
              backPress();
            }}
