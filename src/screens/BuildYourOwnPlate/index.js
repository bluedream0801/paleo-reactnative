...
const SaladsList = [
  ...
  {
    title: 'Sides',
    smallTitle: ' Optional, two included and ฿99 each after that.',
    index: 2,
    data: [
    ],
    numColumns: 2,
...
const BuildYourOwnPlate = (props) => {
        ...
        {token, user_id}
          ...
          (data || []).forEach((meal,i) => {
            ...
            if (meal.comp_type == 'sauce') {
              ...
              sauce_products.push(product_line);
              console.log('sauce_products',sauce_products);
            }

            if (meal.comp_type == 'side') {
              product_line =
              {
                section: 'side',
                heading: meal.product_id.ecom_short_title+' '+meal.product_id.ecom_short_subtitle, // 'Grass fed NZ rump steak 90g serving' // meal.product_id.ecom_short_subtitle
              ...
              side_products.push(product_line);
              console.log('side_products',side_products);
            }
          ...
          console.log('sauce_section',sauce_section);

          newSaladArray.push(sauce_section);

          side_section = {
            title: 'Sides',
            smallTitle: ' Optional, two included for free.',
            index: 2,
            data: side_products,
            numColumns: 2,
            cellWidth: (screenWidth - 34) / 2,
          ...
          console.log('side_section',side_section);

          newSaladArray.push(side_section);
  ...
  const addItemInTopList = (itemObj, i, j) => {
    ...
    if (itemObj.section == 'protein') {
      ...
      if (currentSaladArray[j].data[i]['QuantityAdded'] === 2) {
        currentSaladArray[j].data[i]['QuantityAdded'] = 0;
        console.log('checking 2');
      } else if (currentSaladArray[j].data[i]['QuantityAdded'] == 1) {
        currentSaladArray[j].data[i]['QuantityAdded']++;
      } else {
    ...
    if (itemObj.section == 'side') {
      console.log('checking 1');
      // Maximum of 2
      // Get Addons total quantity
      var total_sides = 0;
      for (k=0;k<currentSaladArray[j].data.length;k++) {
        total_sides += currentSaladArray[j].data[k]['QuantityAdded'];
      }
      ...
      if (total_sides < 2) {
        ...
        alert('You can only have 2 free sides per plate');
      }

      // Can add as many as we want
      // currentSaladArray[j].data[i]['QuantityAdded'] +=1; - Not anymore
    ...
    if (itemObj.section == 'addon') {
      ...
      if (total_addons < 2) {
        ...
        alert('You can only have 2 Premium Addons per plate');
      }
  ...
  const renderHeader = () => {
    ...
    return (
      <View style={[styles.header, { paddingTop: insets.top }]}>
        ...
        >
          Choose a protein and then add up to 2 sides for free.
        </Text>
  ...
  const renderListView = (current_selected_items) => {
    ...
    return (
      <View style={plateStyles.listItems}>
          ...
          {current_selected_items.map((item, index) => {
            return (
              <TouchableOpacity
                disabled={item.price !== ''}
                style={[
                  plateStyles.mixAddedCell,
                  plateStyles.addedCell,
                  {
              ...
              >
                ...
                >
                  <TouchableOpacity
                    style={plateStyles.delContainer}
                    onPress={() => {
                      deletItemFromTopList(item, index)
                    }}
                  >
                    <Image source={delete_ic} style={plateStyles.delImg} />
                  </TouchableOpacity>
                  {item.QuantityAdded == 2 && (
                    <View style={plateStyles.addedQuantity}>
                      <Text
                        minSmall
                        textAlign={'center'}
                        color={white}
                ...
                <View style={plateStyles.addedTexContainer}>
                  <Text
                    small
                    textAlign={'center'}
                    color={accountSettingGray}
                    lineHeight={14}
                    style={plateStyles.cellText}
                    noOfLines={2}
                  >
  ...
  const isEnableCheckoutButton = () => {
    ...
    return { baseItemAdded, no_sides: count_sides }
  }

  const showConfirmDialog = () => {
    return Alert.alert(
      "Are you sure?",
      "You have not added any side dishes. Up to 2 sides are included with your Plate for free.",
      [
        {
          text: "Yes",
          onPress: () => {
  ...
  const AddToCart = async () => {
    ...
    if ( selected_address == "" || user_zone_id == "" || user_zone_id == null || user_zone_id == "null" || user_zone_id == 32 || user_zone_id == 34) {
      ...
      var plate_id = 0;
      var vals={};
      await API.execute("byo.plate", "create", [vals],{},setIsAnyApiLoading, {token, user_id}).then(res=>{
        console.log('Plate id created',res)
        plate_id = res;
      }).catch(err=>{
        alert(err)
      });
      ...
      for (j=0;j<4;j++) {
        for (k=0;k<currentSaladArray[j].data.length;k++) {
          if (currentSaladArray[j].data[k]['QuantityAdded'] > 0) {
            // Add the Protein
            if (currentSaladArray[j].data[k]['section'] == 'protein') {
              vals={type:"protein",product_id:currentSaladArray[j].data[k]['product_id'],qty:currentSaladArray[j].data[k]['QuantityAdded'],plate_id:plate_id}
            ...
            if (currentSaladArray[j].data[k]['section'] == 'sauce') {
              vals={type:"dressing",product_id:currentSaladArray[j].data[k]['product_id'],qty:1,plate_id:plate_id}
              await API.execute("byo.plate.product", "create", [vals],{},setIsAnyApiLoading, {token, user_id}).then(res=>{
                console.log('Sauce Created',res)
              }).catch(err=>{
                alert(err)
              });
            }
            ...
            // Add the dressing
            if (currentSaladArray[j].data[k]['section'] == 'side') {
              vals={type:"side",product_id:currentSaladArray[j].data[k]['product_id'],qty:currentSaladArray[j].data[k]['QuantityAdded'],plate_id:plate_id}
              await API.execute("byo.plate.product", "create", [vals],{},setIsAnyApiLoading, {token, user_id}).then(res=>{
                console.log('Side Created',res)
              }).catch(err=>{
                alert(err)
              });
            }
            ...
            if (currentSaladArray[j].data[k]['section'] == 'addon') {
              vals={type:"premium_addon",product_id:currentSaladArray[j].data[k]['product_id'],qty:currentSaladArray[j].data[k]['QuantityAdded'],plate_id:plate_id}
              await API.execute("byo.plate.product", "create", [vals],{},setIsAnyApiLoading, {token, user_id}).then(res=>{
      ...
      await API.execute("byo.plate", "update_fg_product", [[plate_id]],{},setIsAnyApiLoading, {token, user_id}).then(res=>{
        console.log('FG product created',res)
      }).catch(err=>{
        alert(err)
      });

      var final_plate_product_id = 0;
      await API.execute("byo.plate", "search_read_path", [["id", "=", plate_id ], ['product_id.id']],{},setIsAnyApiLoading, {token, user_id}).then(res=>{
        console.log('FG product details',res)
        console.log('The ID',res[0].product_id.id);
        final_plate_product_id = res[0].product_id.id;
      }).catch(err=>{
        alert(err)
      });
    ...
    for (j=0;j<4;j++) {
      for (k=0;k<currentSaladArray[j].data.length;k++) {
        if (currentSaladArray[j].data[k]['QuantityAdded'] > 0) {
          ...
          if (currentSaladArray[j].data[k]['section'] == 'side') {
            // Do nothing as their free
          } else if (currentSaladArray[j].data[k]['section'] == 'protein' && currentSaladArray[j].data[k]['QuantityAdded'] == 2) {
            final_price+= currentSaladArray[j].data[k]['price'] + currentSaladArray[j].data[k]['double_price'];
          } else {
      ...
      <View style={styles.container}>
        ...
        {showPremiumModal && (
          <View style={styles.modalContain}>
            <PopupModal
              heading={'Maximum one sauce per plate.'}
              content={
                'You can only order 1 sauce per plate.\n(Many of our sauces are also available in\n Grocery sizes in the Sauces section.)'
              }
...
export default BuildYourOwnPlate
